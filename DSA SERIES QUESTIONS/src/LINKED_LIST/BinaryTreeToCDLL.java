...
public Node bTreeToCList(Node root)  {
        if (root==null)
            return null;
    // Recursively convert left and right subtrees to DCLL
...
/*   Make a circular linked list of single node  (or root). To do so, make the right and
        ...
        Thus we wi;l get a Doubly CLL of a single Node in each recursion
* */
        root.left = root;
        root.right = root;
        //OR //root.left = root.right = root;

    return concatenate(concatenate(left,root),right);
    /*  Concatenating each Doubly CLL present in each recursion i.e- ROOT,LEFT CLL,RIGHT CLL
